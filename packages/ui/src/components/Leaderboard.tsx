...
import { getCharacter } from '@otter-river-rush/config';
import {
  formatDate,
  formatDistance,
  getLeaderboard,
  ...
  if (entries.length === 0) {
    return (
      <View className="w-full max-w-[380px] mt-5 mb-2.5">
        <Text className="text-brand-gold text-2xl font-bold text-center mb-3">
          LEADERBOARD
        </Text>
  ...
  return (
    <View className="w-full max-w-[380px] mt-5 mb-2.5">
      ...
      <View className="flex-row border-b-2 border-slate-600 pb-2 mb-1">
        <Text className="text-slate-400 text-xs font-bold uppercase w-[30px] text-center">
          #
        </Text>
        <Text className="text-slate-400 text-xs font-bold uppercase w-[80px] text-right">
          Score
        </Text>
        <Text className="text-slate-400 text-xs font-bold uppercase w-[60px] text-right">
          Dist
        </Text>
        <Text className="text-slate-400 text-xs font-bold uppercase flex-1 pl-2">
          Character
        </Text>
        <Text className="text-slate-400 text-xs font-bold uppercase w-[80px] text-right">
          Date
      ...
      {entries.map((entry) => {
        ...
        return (
          ...
          >
            ...
            <View className="w-[80px] flex-row justify-end items-center">
              ...
              {isHighlighted && (
                <View className="bg-brand-gold ml-1.5 px-1.5 py-0.5 rounded">
                  <Text className="text-brand-surface text-[10px] font-bold uppercase">
                    NEW
                  </Text>
                </View>
              )}
            ...
            <Text
              className="text-slate-200 text-sm flex-1 pl-2"
              numberOfLines={1}
            >
