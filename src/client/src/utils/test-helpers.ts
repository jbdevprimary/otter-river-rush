...
export function expectPosition(entity: Entity, x: number, y: number, tolerance: number = 0.1) {
  expect(Math.abs(entity.position.x - x)).toBeLessThan(tolerance);
  expect(Math.abs(entity.position.y - y)).toBeLessThan(tolerance);
}

export function expectVelocity(entity: Entity, x: number, y: number, tolerance: number = 0.1) {
  if (!entity.velocity) {
