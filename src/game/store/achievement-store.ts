...
export interface AchievementState {
  ...
  checkAchievements: (stats: Partial<Record<AchievementTrackingKey, number>>) => AchievementNotification[];
    ...
    (set, get) => ({
      ...
      endSession: (finalStats) => {
        ...
        const survivalTimeSeconds = state.sessionTracking.damageTaken
          ? Math.floor((state.sessionTracking.lastDamageTime - state.sessionTracking.gameStartTime) / 1000)
        ...
        newLifetimeProgress.highScore = Math.max(
          newLifetimeProgress.highScore ?? 0,
          finalStats.score
        );
      ...
      getProgress: (achievementId) => {
        ...
        if (definition.trackingScope === 'session') {
          const sessionValue = state.sessionTracking[
            definition.trackingKey as keyof SessionTracking
          ];
