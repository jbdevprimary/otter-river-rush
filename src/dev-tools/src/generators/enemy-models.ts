...
async function generateEnemyModels() {
  ...
  for (const enemy of ENEMY_DEFINITIONS) {
    ...
    try {
      ...
      console.log(
        `✅ ${enemy.name}: Generated (${(result.modelData.length / 1024 / 1024).toFixed(2)} MB)`
      );
    } catch (error) {
  ...
  writeFileSync(join(OUTPUT_DIR, 'enemy-models-manifest.json'), JSON.stringify(manifest, null, 2));
